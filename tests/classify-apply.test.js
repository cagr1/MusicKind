...
test("mueve a nombre único, escribe manifiesto y permite deshacer", async (t) => {
  ...
  const applied = await applyClassifyMoves({ moves: [{ from, to: desired }], destRoot: env.dest });
...
test("rechaza lote inválido antes de mover cualquier archivo", async (t) => {
  const env = setup(); t.after(() => fs.rmSync(env.root, { recursive: true, force: true }));
  const safe = env.add(env.input, "safe.wav");
  await assert.rejects(applyClassifyMoves({ destRoot: env.dest, moves: [{ from: safe, to: path.join(env.dest, "safe.wav") }, { from: path.join(env.root, "missing.wav"), to: path.join(env.dest, "missing.wav") }] }), /Origen inexistente/);
  assert.equal(fs.existsSync(safe), true); assert.equal(fs.existsSync(env.dest), false);
  await assert.rejects(applyClassifyMoves({ destRoot: env.dest, moves: [{ from: safe, to: path.join(env.root, "outside.wav") }] }), /fuera de destRoot/);
});
...
test("cancelar tras el primer archivo deja un manifiesto coherente", async (t) => {
  ...
  const result = await applyClassifyMoves({ destRoot: env.dest, moves: [a, b].map((from) => ({ from, to: path.join(env.dest, path.basename(from)) })), cancelled: () => stop, onProgress: () => { stop = true; } });
...
test("deshacer ocupado no sobrescribe origen y EXDEV copia/verifica/borra", async (t) => {
  ...
  const applied = await applyClassifyMoves({ destRoot: env.dest, moves: [{ from, to: path.join(env.dest, "track.flac") }], renameSync: forceExdev });
...
test("API valida antes de mover y aplica/deshace por SSE", async (t) => {
  const env = setup(); t.after(() => fs.rmSync(env.root, { recursive: true, force: true }));
  const from = env.add(env.input, "track.mp3");
  const settingsFile = path.join(env.root, "settings.json"); fs.writeFileSync(settingsFile, "{}");
  const server = createServer({ settingsFile });
  await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve));
  t.after(() => new Promise((resolve) => server.close(resolve)));
  const base = `http://127.0.0.1:${server.address().port}`;
  const post = (url, body) => fetch(`${base}${url}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
  let response = await post("/api/classify-apply", { destRoot: env.dest, moves: [
    { from, to: path.join(env.dest, "track.mp3") }, { from: path.join(env.root, "absent.wav"), to: path.join(env.dest, "absent.wav") }
  ] });
  assert.equal(response.status, 400); assert.equal(fs.existsSync(from), true); assert.equal(fs.existsSync(env.dest), false);
  response = await post("/api/classify-apply", { destRoot: env.dest, moves: [{ from, to: path.join(env.dest, "track.mp3") }] });
