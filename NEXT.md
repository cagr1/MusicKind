...
1. [x] Re-medir afinidad tras S3b (2026-09-25). `scripts/eval_sets.py` estaba roto desde S1 (`score_sections` devuelve 4 valores); Luna lo adaptó (+ métricas «Por revisar» y «acierto sobre decididas»), sin commit. Misma lógica S1, 85 sondeos: **caché librosa 36,5 % (decididas 42,5 %, por revisar 14,1 %) → caché S3b/Percival 43,5 % (decididas 50,0 %, por revisar 12,9 %)**, azar 33 %. El 44,7 % viejo no es comparable (antes de S1 un empate/cero se asignaba igual). Reporte: `.cache/eval/sets-report-s3b.md`. Sigue siendo separación de géneros, no calidad de set.
1b. [x] **X1/X1.1** Sets → botón «Exportar playlists»: `<entrada> - Warmup/Peak/Closing/Por revisar.m3u8` (orden en pantalla, rutas absolutas, 409 + confirmar para sobrescribir). Luna 2 corridas (X1.1: errores ≠409 se tragaban). Verificado 2026-09-25: node 108/108, vitest 108/108, build/lint/format 0; en vivo 3099 sobre copias: 200/409/overwrite/400 pista inexistente/400 bajo 2026, ffprobe abre todas las rutas, VLC carga el playlist y reproduce la 1.ª pista en orden. Sin QA del botón en Electron (Carlos). `duration` va −1 (Sets no la tiene).
1c. **Prueba real de Carlos (2026-09-25): 0 pistas a Peak.** Refs warmup=`muzrec/Deep` (20), peak=`muzrec/Tech House` (81), closing=`MUSIC BACKUP/Music/House` (77); target `MUSIC BACKUP/Test` (766). Reproducido por CLI sobre 117 pistas (30 por subcarpeta): warmup 16, peak 0, closing 97, por revisar 4. **Causa verificada:** `score_sections` (`src/style_analyzer.py:80`) puntúa por percentil contra la dispersión de cada carpeta; peak es la más compacta (mediana 5,3 vs closing 7,3) → nunca gana. Con centro más cercano: peak 25/117; con k-NN (k=7): peak 56/117 (sesgado por tamaño; warmup 2). **Ningún método separa los géneros de Test**: cada subcarpeta se reparte parecido → las features de timbre no distinguen Afro/Deep/Tech House de estas refs. Tech House de Test va a 121–128 BPM (refs peak 127–130). Vectores en scratchpad `diag/vecs.pkl` (temporal). Decisión pendiente de Carlos.
1d. [x] **U1** hueco sobre el encabezado fijo (causa: `py-2` en el contenedor con scroll) + **U2** ordenar columnas en todas las tablas (Sets: dentro de cada sección; cola, selección y exportación siguen el orden visible). Luna 1 corrida. Verificado 2026-09-25: vitest 113/113, build/lint/format 0; en vivo (Chrome headless, 3099, Metadatos con 81 pistas reales): con scroll el `thead` queda pegado al borde del contenedor (0 px de hueco), clic en Pista → asc/desc/original correcto con `aria-sort`. Sets no probado en vivo.
1e. [x] **S1.2/S1.3** puntaje de Sets: escala común para las 3 secciones + ganador = centro más cercano; «Por revisar» solo si lejos de todo. Luna 2 corridas (S1.2 sola subía por revisar a 30,6 % → corregido en S1.3). Verificado 2026-09-25: unittest 24/24, node 108/108; muestra de Carlos (117): peak 0 → 23, por revisar 4; validación cruzada con sus carpetas (15 refs, 10 semillas) 31,3 % → 51,3 % (azar 33 %); `2026` 43,5 % igual, por revisar 12,9 → 11,8 % (`.cache/eval/sets-report-s13.md`). Sigue sin separar subgéneros → foco pasa a S3 (orden por BPM/Camelot).
1f. [x] **S3a** «Ordenar set» por BPM/Camelot dentro de cada sección, con motivo por transición (`src/set-sequencer.js`, `/api/set-sequence`, modo secuencia en Sets). Luna 1 corrida. Verificado 2026-09-25: node 115/115, vitest 114/114, build/lint/format 0; sobre los 117 resultados reales: paso medio 1,7 BPM, 8 saltos >3 BPM, choques de tonalidad closing 72 → 10 frente a ordenar solo por BPM, 31 ms. Barrido de pesos (bajar ×1,5–5, choque 2–4) no mejora → se dejan. **Sin QA visual del modo secuencia** (requiere análisis en la app; Carlos).
1g. [x] **U3** columna «Artista» ordenable en todas las tablas (pedido de Carlos). Luna 1 corrida. Verificado 2026-09-25: vitest 115/115, build/lint/format 0; en vivo Metadatos (81 pistas reales): columna visible sin scroll horizontal con Inspector abierto, orden asc/desc/original correcto. Otras vistas solo por tests/build.
1h. [x] **U4–U4.8** tablas responsive (Inspector como panel bajo 1100 px, Artista como columna o 2.ª línea según ancho, sin superposición), quitar filas + deshacer en Clasificador por etiquetas (las quitadas no se mueven), Convertidor sin «General», Stems (destino en vez del nombre duplicado, play circular y tiempo junto a los carriles). Luna 5 corridas (U4.5–U4.8 por fallos hallados en capturas del cerebro). Verificado 2026-09-26: vitest 116/116, build/lint/format 0; capturas Chrome headless a 900/1100/1300 px de Convertidor, Metadatos y Stems sin superposición. Quitar filas del Clasificador solo revisado en código + tests (sin QA en vivo).
1i. [x] **M1** identificar: `RUN DMC`=`Run-DMC`, quitar «Unrelease», sugerir la original si la versión no existe (`matchType: original`, título con la versión conservado), error con motivos, lote que no se corta. Luna 1 corrida. Verificado 2026-09-26: node 121/121, vitest 116/116 (falta test vitest del lote; verificado en vivo), build/format 0; en vivo con copia del archivo de Carlos: «It's Like That (Raxon Edit)» · RUN DMC, Jason Nevins · Best Of 2007 marcado «Original, no esta versión», archivo sin cambios (md5 igual); lote con un fallo primero sigue y marca «No identificada» con motivos.
1j. [x] **B1/B1.1/B1.2** BPM: lo analizado sin tag queda «sin guardar» con botón Guardar (antes se tomaba como ya guardado y nunca se escribía), columna de acciones completa, un solo contador junto a la carpeta. Luna 3 corridas. Verificado 2026-09-26: vitest 120/120, build/lint/format 0; en vivo (copia sin tags): analizar → «Guardar cambios 1» → archivo con TBPM 127 / TKEY Cm; al volver a cargarlo, ✓ sin pendiente; capturas 1100/1300 px.
1k. [x] **M2** Metadatos: identify conserva «(Extended Mix)»/«(Original Mix)» (regresión de M1) y «Guardar cambios N» escribe tags + renombra todas las filas pendientes (salta «Original, no esta versión», no corta el lote). Luna 1 corrida. Verificado 2026-09-26: node 124/124, vitest 122/122, build/lint/format 0; en vivo con copias: `track.aiff` → `Italobros - Excuse Me (Extended Mix).aiff` con tags; RUN DMC (original) no se toca; toast «1 guardadas · 0 con error · 1 por revisar». Nota: el `track.aiff` real de Carlos sigue sin renombrar (identify solo propone; faltaba pulsar Guardar).
...
9. [~] **Lote 6 — Clasificar por ejemplos** (`design/SPEC.md` § Lote 6). **PAUSADO 2026-09-24 a pedido de Carlos.**
   ...
   - [x] **E4/E4.1** playlists `Warmup/Peak/Closing.m3u8` (rutas relativas, orden por BPM normalizado a 85–175, ilegibles omitidas) desde la vista Clasificador. Verificado 2026-09-25: node 98/98, vitest 100/100, build/lint/format 0; vista previa real sobre `MUSIC BACKUP` (solo lectura, carpetas viejas): 277/462/145 pistas en 35 s, nada escrito en el disco; escritura en copia: ffprobe abre todas las rutas, archivo corrupto omitido; diálogo verificado en captura. `Clasificado/` aún no existe (etapa 1 sin aplicar por Carlos).
   - [x] **E2.4** lista del clasificador sobrevive al cambio de pestaña (en memoria hasta limpiar/nuevo análisis). Verificado en vivo 2026-09-25: cambiar de pestaña en pleno análisis y volver → 7/7 filas, dos veces; vitest 101/101. Carlos probó 980 archivos reales: 980 bien clasificados en 2–3 min (lo grabó).
   - [x] **G1/G1.1/G1.2** géneros para cualquier música: catálogo Discogs de fábrica (15 familias, ~370 estilos) + catálogo aprendido (`<dataDir>/genre-catalog-learned.json`, solo desde Discogs), familia en cada fila y estado "Solo familia", analizar sin destino, archivos sueltos (`inputPaths`). Verificado 2026-09-25: node 107/107, vitest 103/103; de 92 estilos reales de Discogs en caché, 0 sin resolver (2 son familia/genérico por diseño); en vivo Salsa/Reggaeton/Tropical House con género, Pop/Rock solo familia, **"Juan Luis Guerra - Bachata Rosa" sin tag → Bachata (Discogs)**, `track01.mp3` → por revisar. Siguiente: G2 (Configuración → Géneros) y medir audio para familia (CLAP) como último respaldo.
   - [-] E0b descartado por Carlos 2026-09-25 (costo > beneficio: ~92 pistas por revisar se clasifican a mano). Pendiente después (texto viejo): E0b (entrenar audio con las ~2,300 etiquetadas para sugerir en las ~190 sin tag).
...
**C2 [x]** Convertidor: lista inmediata + formato por fila (pedido de Carlos 2026-09-25; spec `design/SPEC.md` § C2–C2.3.1; Luna, 5 corridas).
Verificado por el cerebro en vivo (Chrome headless + backend 3099, copias): carpeta con subcarpetas → lista antes de convertir; a.flac en AIFF y resto WAV → `out/House/a.aiff`, `out/Techno/b.wav`, archivo suelto plano `out/3.wav`, WAV omitido. Bugs hallados en QA y corregidos: carpeta soltada no se expandía + bucle infinito en `commonParent` (C2.1); archivo suelto con raíz = sí mismo escribía `<output>.aiff` **fuera** de la carpeta de salida (C2.2, ahora validado en Python); pulido visual (C2.3). vitest 99/99, node 93/93, Python (venv) 4 suites OK, build/lint/format 0. Falta: soltar de verdad en Electron (Carlos).
**S1/S1.1 [x]** (creador de sets, orden en `plan.md` § Orden único): afinidad sin `%`, grupo "Por revisar" (sin parecido / empate / error; `best` null), Inspector explica la escala con N referencias, curva de "energía" falsa eliminada; bug viejo de Back 2 corregido (`/api/metadata` pisaba el BPM analizado → columna en "—"). Verificado 2026-09-25: Python 7/7, vitest 103/103, build/lint/format 0; en vivo 20 pistas / 8 refs por sección: 5 peak, 6 closing, 9 por revisar (60 s). **Hallazgo para S3:** librosa da BPM cuantizados (129.2, 123, 117.5… muchas pistas en 129.2) → antes de ordenar por BPM hay que usar BPM del tag cuando exista y mejorar la resolución del análisis.
**D2 [x]** soltar/elegir uniforme (pedido de Carlos): BPM y Metadatos aceptan varios archivos y carpetas recursivas, suman sin reemplazar (Metadatos conserva ediciones), BPM muestra pendientes antes de analizar; helper común `web/src/lib/drop.ts`. Verificado 2026-09-25 con arrastre simulado (CDP): BPM, Metadatos y Convertidor 2 → 5 filas; vitest 107/107, build/lint/format 0.
**S3a [x]** medición BPM (`scripts/eval_bpm.py`, 195 pistas de `2026`, verdad = BPM del tag; repetida desde caché idéntica): librosa actual **10,3% ±0.5** (70% de las pistas en 129.2); **essentia Percival pista entera 98,5% ±0.5 / 99% ±1, 2,4 s/pista** (60 s: 96,9%, 1,3 s); essentia Rhythm 94%, 15,7 s; librosa-fine descartado (doble tempo). essentia es AGPL-3 (compatible con GPL-3; anotar en THIRD_PARTY_NOTICES). Siguiente: **S3b** = tag primero → Percival pista entera → librosa como último recurso, en `bpm_analyzer.py`, `style_analyzer.py`, `audio_features.py`.
**S3b [x]** BPM confiable (spec `design/SPEC.md` § S3b; Luna, 1 corrida): nuevo `src/bpm_detection.py` (tag → essentia Percival pista entera → librosa), usado por `bpm_analyzer.py`, `style_analyzer.py` (vector y salida) y `audio_features.py`; salidas con `bpmSource`; essentia en `requirements.txt` y THIRD_PARTY_NOTICES (AGPL-3.0). Verificado 2026-09-25: unittest venv 22/22, node 104/104; en vivo (3099, 12 copias de `2026` + las mismas sin metadatos): con tag = valor del tag/`tag`; sin tag 12/12 dentro de ±1 del tag (Percival); `/api/set-analyze` 12/12 con BPM y `bpmSource`, sin errores (~5 s/pista con timbre y tonalidad). Re-medido: ver «AL RETOMAR» punto 1 (36,5 % → 43,5 % con la misma lógica). UI aún no muestra `bpmSource`. Después: prueba con carpetas warmup/peak/closing reales de Carlos (≥20 c/u): acierto vs nº de referencias (5/10/20) + escucha a ciegas.
**Siguiente (2026-09-25, decisión de Carlos):** antes de entregar, pulir la app: **creador de sets** (la "estrella"; hoy solo puntúa parecido, 44,7% vs azar 33%; falta ordenar por Camelot/BPM/energía — esperando qué espera Carlos) + QA de bugs. Punto final: P1 [x] (binarios ffmpeg/ffprobe 9.0.2 estáticos Martin Riedl GPL-3, fpcalc 1.5.1, `scripts/fetch-binaries.sh <arm64|x64>`, binarios fuera de git; `electron/runtime-path.cjs` añade Resources/bin + Homebrew al PATH; verificado: lipo/otool, x64 bajo Rosetta, node 101/101 — ojo: `tests/runtime-path.test.cjs` no entra en el glob `*.test.js`); **P2/P3 (Python propio, .dmg, release) EN PAUSA** hasta el OK de Carlos. QA de Carlos en Electron cuando pueda.
