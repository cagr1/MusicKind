...
**S3b [x]** BPM confiable (spec `design/SPEC.md` § S3b; Luna, 1 corrida): nuevo `src/bpm_detection.py` (tag → essentia Percival pista entera → librosa), usado por `bpm_analyzer.py`, `style_analyzer.py` (vector y salida) y `audio_features.py`; salidas con `bpmSource`; essentia en `requirements.txt` y THIRD_PARTY_NOTICES (AGPL-3.0). Verificado 2026-09-25: unittest venv 22/22, node 104/104; en vivo (3099, 12 copias de `2026` + las mismas sin metadatos): con tag = valor del tag/`tag`; sin tag 12/12 dentro de ±1 del tag (Percival); `/api/set-analyze` 12/12 con BPM y `bpmSource`, sin errores (~5 s/pista con timbre y tonalidad). **Pendiente:** re-medir `scripts/eval_sets.py --cache .cache/eval/sets-s3b --report .cache/eval/sets-report-s3b.md` (el disco se desmontó) y comparar con 44,7 %. UI aún no muestra `bpmSource`. Después: prueba con carpetas warmup/peak/closing reales de Carlos (≥20 c/u): acierto vs nº de referencias (5/10/20) + escucha a ciegas.
