...
9. [~] **Lote 6 — Clasificar por ejemplos** (`design/SPEC.md` § Lote 6). **PAUSADO 2026-09-24 a pedido de Carlos.**
   ...
   - [x] **Back 4 / 4.1–4.3** — Spotify eliminado (feb-2026: sin `genres` y exige Premium). Proveedores en `src/providers/`: Discogs (styles), Last.fm, Deezer, MusicBrainz; claves de la app en `config/app-keys.json` (gitignored, repo público) → el usuario no crea claves. Online ignora genéricos (Electronic/dance).
     Verificado 2026-09-24: node 86/86, build/lint 0, vitest 89/89; en vivo `Unsorted`: 1197 tag + 37 Discogs + 5 Last.fm, 92 por revisar. AcoustID: clave válida (verificado 2026-09-24, `status: ok`). Deezer (4.4) verificado en vivo: "Fisher – Losing It" → ISRC/BPM/carátula. **Pendiente 4.5:** con varios artistas en el tag ("&ME, Black Coffee, Keinemusik") no encuentra; probar con cada artista por separado.
...
**Siguiente (2026-09-24):** Back 4.5 (varios artistas en Deezer) → remixer en `identify` → L2 Configuración.
